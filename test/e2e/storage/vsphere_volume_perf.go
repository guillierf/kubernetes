...
var _ = SIGDescribe("vcp-performance [Feature:vsphere]", func() {
	...
	It("vcp performance tests", func() {
		...
		framework.Logf("Creating %v PVCs and waiting for bound phase: %v seconds", volumeCount, sumLatency[0]/iterations64)
		framework.Logf("Creating %v Pod: %v seconds", volumeCount/volumesPerPod, sumLatency[1]/iterations64)
		framework.Logf("Deleting %v Pod and waiting for disk to be detached: %v seconds", volumeCount/volumesPerPod, sumLatency[2]/iterations64)
		framework.Logf("Deleting %v PVCs: %v seconds", volumeCount, sumLatency[3]/iterations64)
...
func invokeVolumeLifeCyclePerformance(f *framework.Framework, client clientset.Interface, namespace string, sc []*storageV1.StorageClass, volumesPerPod int, volumeCount int, nodeSelectorList []*NodeSelector) []int64 {
	...
	latency = append(latency, int64(elapsed.Seconds()))
